...
function doGetContests() {
  backend
      .getContests(function(result) {
        if (result.success) {
          var content = "<thead><tr><th>ID</th><th>Name</th><th>Begin time</th><th>End time</th></thead><tbody>";
          var length = result.length;
          for (var i = 0; i < length; i++) {
            content += "<tr>" + "<td>" + result.cid[i] + "</td>" + "<td>" +
                result.name[i] + "</td>" + "<td>" +
                new Date(result.begin[i]).toLocaleString() + "</td>" + "<td>" +
                new Date(result.end[i]).toLocaleString() + "</td>" + "</tr>";
            if (i == 9)
          ...
          popup_noti("<span style='color:red'>Get contests failed: " +
              result.reason + "</span>");
        }
      });
}*/
...
app.controller("list_contest_ctrl", [ "$scope", "backend",
