...
function doAddProblem() {
  backend.addProblem($("#add-edit-problem-modal #name").val(), $(
      "#add-edit-problem-modal #desc").val(), $(
      "#add-edit-problem-modal #input").val(), $(
      "#add-edit-problem-modal #output").val(), add_problem_session, $(
      "#add-edit-problem-modal #num-test-cases").val(), $(
      "#add-edit-problem-modal #type").val(), function(result) {
    if (result.success) {
      popup_noti("Add problem successfully");
    } else {
      popup_noti("<span style='color:red'>Add problem failed: " +
          result.reason + "</span>");
    }
  });
}
  ...
  backend.edit_problem(
      ...
      function(result) {
        if (result.success) {
          ...
          popup_noti("<span style='color:red'>Edit problem failed: " +
              result.reason + "</span>");
        }
      });
}
      ...
      for (var i = 0; i < length; i++) {
        content+=
          "<tr class='problem-row' pid='"+pid[i]+"' name='"+name[i]+"'>"+
            "<td>"+pid[i]+"</td>"+
            "<td>"+name[i]+"</td>"+
            "<td width='32'><span class='glyphicon glyphicon-wrench edit need-admin'></span></td>"+
            "<td width='32'><span class='glyphicon glyphicon-remove remove need-admin'></span></td>"+
          "</tr>";
      }
      content+="</tbody>";
...
function doRemoveProblem(pid) {
  backend.removeProblem(pid, function(result) {
    if (result.success)
      doGetProblems();
    else
      popup_noti("<span style='color:red'>Remove problem failed: " +
          result.reason + "</span>");
  });
}
function doSubmitCode(username, pid, code) {
  backend.submitCode(username, pid, code, function(result) {
    if (result.success) {
      if (result.score == 100) {
        popup_noti("<span style='color:green'>P" + result.pid +
            ": Accepted</span>");
      } else {
        popup_noti("<span style='color:yellow'>P" + result.pid + ": " +
            result.result + " " + result.score + "/100</span>");
      }
    } else {
      popup_noti("<span style='color:red'>Submit code failed: " +
          result.reason + "</span>");
    }
  });
}

$("body").undelegate('#add-edit-problem-modal', "hidden.bs.modal");
$("body").delegate('#add-edit-problem-modal', "hidden.bs.modal", function(e) {
...
$("body").undelegate("#add-edit-problem-modal #num-test-cases", "change");
$("body").delegate(
    "#add-edit-problem-modal #num-test-cases",
    "change",
    function(e) {
      $("#add-edit-problem-modal #missing-files").text('all');
      $("#confirm-add-problem-btn").addClass("disabled");
...
$('#test-cases-upload').fileupload(
    {
      ...
      formData : function() {
        return [ {
          ...
          name : 'max_case',
          value : $("#add-edit-problem-modal #num-test-cases").val()
        } ];
      },
      ...
      progress : function(e, data) {
        if (data.loaded == data.total) {
          $("#add-edit-problem-modal #upload-info")
...
$("body").undelegate('.problem-row', 'click');
...
$("body").undelegate('.problem-row .edit', 'click');
$("body").delegate(
    '.problem-row .edit',
    'click',
    function(e) {
      showEditProblem(e.currentTarget.parentElement.parentElement
          .getAttribute("pid"));
      e.stopPropagation();
    });

$("body").undelegate('.problem-row .remove', 'click');
$("body").delegate(
    '.problem-row .remove',
    'click',
    function(e) {
      showConfirm("Confirm", "Are you sure to remove the problem?", function() {
        doRemoveProblem(e.currentTarget.parentElement.parentElement
            .getAttribute("pid"));
      }, null, "btn-danger");
      e.stopPropagation();
    });

$("body").undelegate(".show-submit-problem-btn", "click");
$("body").delegate(".show-submit-problem-btn", "click", function(e) {
  if (backend.loginLevel == 0) {
    popup_noti("You need login first");
    return;
  }
  var pid = e.currentTarget.getAttribute("pid");
...
$("body").undelegate(".cancel-submit-problem-btn", "click");
...
$("body").undelegate(".confirm-submit-problem-btn", "click");
...
onactive["problems"] = doGetProblems;
enableTab("problems");
